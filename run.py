...
# """
# jsub -q qgpu -J edcstfn -m gpu01 -e error.txt -o output.txt python run.py --lr 1e-3 --num_workers 12 --batch_size 12 --epochs 50 --cuda --ngpu 4 --image_size 2040 1720 --patch_size 512 --patch_stride 120 --save_dir out --train_dir data/train --val_dir data/val --test_dir data/val
# """
...
parser.add_argument('--save_dir', type=Path, default=Path(r'D:\研究生\实验室项目代码\EWMT_code\out'), help='the output directory')
...
parser.add_argument('--train_dir', type=Path, default=(r'D:\研究生\实验室数据集\时空融合\CIA1\train'), help='the training data directory')
parser.add_argument('--val_dir', type=Path, default=(r'D:\研究生\实验室数据集\时空融合\CIA1\test'), help='the validation data directory')
parser.add_argument('--test_dir', type=Path, default=(r'D:\研究生\实验室数据集\时空融合\CIA1\testt'), help='the test data directory')
...
parser.add_argument('--patch_size', type=int, nargs='+', default=128, help='the coarse image patch size for training restore')
